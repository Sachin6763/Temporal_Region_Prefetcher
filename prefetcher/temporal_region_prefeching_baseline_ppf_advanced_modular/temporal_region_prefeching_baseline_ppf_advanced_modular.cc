// Modularized, Complex Hybrid Prefetcher
...
void update_weights(RegionPair& pair, const std::array<int, 3>& features, int label) {
    for (size_t i = 0; i < features.size(); ++i) {
        perceptron_weights[pair][i] += label * features[i] * 0.2;
...
void perform_prefetch(uint64_t base_addr, const std::unordered_set<int>& deltas, CACHE* cache) {
    ...
    if (it != from_region_map.end()) {
        for (const auto& pair : it->second) {
            ...
            if (score >= dynamic_threshold) {
                ...
                perform_prefetch(base_addr, spatial_delta_map[pair], this);
            }
        }
    }
